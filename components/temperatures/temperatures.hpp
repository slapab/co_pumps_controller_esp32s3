...
/** Supported sensors.
 * @note used also for array indexing
 */
enum class temp_sensor_t : uint8_t {
    GROUND_FLOOR = 0,
    FLOOR_1,

    SENSORS_NUM /**< Suported sensors number */
};
...
public:
    ...
    float get_temp(const temp_sensor_t sensor_id) { return temps[static_cast<uint8_t>(sensor_id)]; }
